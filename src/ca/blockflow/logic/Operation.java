...
public enum Operation {
    ...
    NOT((l, r) -> {
        ...
        return new Variable<>(("!" + lVal), SupportedTypes.BOOLEAN, ! lVal);
    }),
    
    AND((l, r) -> {
        boolean lVal = booleanVal(l);
        boolean rVal = booleanVal(r);
        return new Variable<>((lVal + " && " + rVal), SupportedTypes.BOOLEAN, Boolean.logicalAnd(lVal, rVal));
    }),
    
    OR((l, r) -> {
        boolean lVal = booleanVal(l);
        boolean rVal = booleanVal(r);
        return new Variable<>((lVal + " && " + rVal), SupportedTypes.BOOLEAN, Boolean.logicalOr(lVal, rVal));
    }),
    
    XOR((l, r) -> {
        boolean lVal = booleanVal(l);
        boolean rVal = booleanVal(r);
        return new Variable<>((lVal + " && " + rVal), SupportedTypes.BOOLEAN, Boolean.logicalXor(lVal, rVal));
    }),

    CONCAT((l, r) -> {
        Object lVal = genericVal(l);
        Object rVal = genericVal(r);
        return new Variable<>((lVal + " + " + rVal), SupportedTypes.STRING, concat(lVal, rVal));
    }),
    INDEX_SEARCH((l, r) -> {
        Object lVal = genericVal(l);
        int rVal = intValue(r);
        return new Variable<>((lVal + "[" + rVal + "]"), SupportedTypes.STRING, indexSearch(lVal, rVal));
    });
